...
- `fleet policy`: shows, toggles, scopes, requires, relaxes, edits, clears, and previews the optional operator constitution.
- Optional `constitution` config block: prepends customizable operator rules to `fleet task`, `fleet parallel`, and `fleet steer` dispatches, with optional required mode that blocks scoped dispatches when rules are disabled or empty.
